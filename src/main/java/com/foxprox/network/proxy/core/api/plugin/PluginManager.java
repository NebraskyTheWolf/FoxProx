...
import com.foxprox.network.proxy.core.event.EventBus;
import com.foxprox.network.proxy.core.event.EventHandler;
import com.foxprox.network.proxy.core.event.EventHandlerMethod;
import com.foxprox.network.proxy.lib.event.ProxyExceptionEvent;
import com.foxprox.network.proxy.lib.exception.ProxyCommandException;
import com.foxprox.network.proxy.lib.exception.ProxyEventException;
import com.foxprox.network.proxy.lib.exception.ProxyPluginEnableDisableException;
import com.foxprox.network.proxy.lib.exception.ProxyTabCompleteException;
...
public final class PluginManager
{
    ...
    public PluginManager(FoxServer proxy)
    {
        this.proxy = proxy;
        ...
        eventBus = new EventBus( new Logger("EventBus"));
    }
    ...
    private Command getCommandIfEnabled(String commandName, CommandSender sender)
    {
        ...
        if ( ( sender instanceof FoxPlayer) && proxy.getDisabledCommands().contains( commandLower ) )
        {
            return null;
        }
    ...
    private boolean enablePlugin(Map<PluginDescription, Boolean> pluginStatuses, Stack<PluginDescription> dependStack, PluginDescription plugin)
    {
        ...
        if ( status )
        {
            try
            {
                URLClassLoader loader = new PluginClassloader( proxy, plugin, plugin.getFile(), null );
    ...
    public <T extends Event> void handleEventException(String msg, T event, EventHandlerMethod method, Throwable ex) {
