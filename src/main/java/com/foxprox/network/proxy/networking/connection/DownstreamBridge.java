...
import com.foxprox.network.proxy.core.api.score.Objective;
import com.foxprox.network.proxy.core.api.score.Position;
import com.foxprox.network.proxy.core.api.score.Score;
import com.foxprox.network.proxy.core.api.score.Scoreboard;
import com.foxprox.network.proxy.lib.event.ProxyDefineCommandsEvent;
...
import com.foxprox.network.proxy.ServerConnection;
import com.foxprox.network.proxy.ServerConnection.KeepAliveData;
...
import com.foxprox.network.proxy.core.api.event.PluginMessageEvent;
import com.foxprox.network.proxy.core.api.event.ServerConnectEvent;
import com.foxprox.network.proxy.core.api.event.ServerDisconnectEvent;
import com.foxprox.network.proxy.core.api.event.ServerKickEvent;
import com.foxprox.network.proxy.core.api.event.TabCompleteResponseEvent;
...
public class DownstreamBridge extends PacketHandler
{
    ...
    public void handle(Team team) throws Exception
    {
        ...
        com.foxprox.network.proxy.core.api.score.Team t;
        if ( team.getMode() == 0 )
        {
            t = new com.foxprox.network.proxy.core.api.score.Team( team.getName() );
    ...
    public void handle(PluginMessage pluginMessage) throws Exception
    {
        ...
        if ( pluginMessage.getTag().equals( "BungeeCord" ) )
        {
            ...
            switch ( subChannel )
            {
                ...
                {
                    FoxPlayer target = bungee.getPlayer( in.readUTF() );
                    if ( target != null )
                    {
                ...
                case "ConnectOther":
                {
                    FoxPlayer player = bungee.getPlayer( in.readUTF() );
                    if ( player != null )
                    {
                        ServerInfo server = bungee.getServerInfo( in.readUTF() );
                        if ( server != null )
                ...
                case "Message":
                {
                    ...
                    if ( target.equals( "ALL" ) )
                    {
                        for ( FoxPlayer player : bungee.getPlayers() )
                        {
                            player.sendMessage( message );
                        }
                    } else
                    {
                        FoxPlayer player = bungee.getPlayer( target );
                        if ( player != null )
                        {
                            player.sendMessage( message );
                        }
